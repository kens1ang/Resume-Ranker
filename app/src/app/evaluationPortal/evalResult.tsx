...
import { Loader2, FileText, BarChart, ArrowUpDown, ChevronDown, ChevronUp } from "lucide-react";
...
export function EvalResult({ selectedJob, onChangePosition }: EvalResultProps) {
  ...
  useEffect(() => {
    async function fetchResults() {
      ...
      try {
        // Convert job title to collection name (same logic as in resumeUpload)
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground max-w-md mt-2">
            There are no evaluation results for this position yet. Try uploading some resumes first.
          </p>
        </div>
      ) : (
          ...
          <div className="bg-muted/30 p-4 rounded-lg">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white p-4 rounded-lg border shadow-sm">
                <div className="text-sm font-medium text-muted-foreground mb-1">Total Resumes</div>
                <div className="text-2xl font-bold">{results.length}</div>
              </div>
              <div className="bg-white p-4 rounded-lg border shadow-sm">
                <div className="text-sm font-medium text-muted-foreground mb-1">Avg. Overall Match</div>
                <div className="text-2xl font-bold">
                  {formatPercentage(results.reduce((acc, r) => acc + r.similarityScores.overall, 0) / results.length)}
                </div>
              </div>
              <div className="bg-white p-4 rounded-lg border shadow-sm">
                <div className="text-sm font-medium text-muted-foreground mb-1">Top Match</div>
                <div className="text-2xl font-bold text-green-600">
                  {formatPercentage(Math.max(...results.map(r => r.similarityScores.overall)))}
                </div>
              </div>
              <div className="bg-white p-4 rounded-lg border shadow-sm">
                <div className="text-sm font-medium text-muted-foreground mb-1">Avg. Skills Match</div>
                <div className="text-2xl font-bold">
                  {formatPercentage(results.reduce((acc, r) => acc + r.similarityScores.skills, 0) / results.length)}
                </div>
              </div>
            </div>
          </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="cursor-pointer" onClick={() => toggleSort("overall")}>
                    <div className="flex items-center">
                      Overall Match
                      <ArrowUpDown className="ml-2 h-4 w-4" />
                    </div>
                  </TableHead>
                  <TableHead className="cursor-pointer hidden md:table-cell" onClick={() => toggleSort("skills")}>
                    <div className="flex items-center">
                      Skills
                      <ArrowUpDown className="ml-2 h-4 w-4" />
                    </div>
                  </TableHead>
                  <TableHead className="cursor-pointer hidden md:table-cell" onClick={() => toggleSort("education")}>
                    <div className="flex items-center">
                      Education
                      <ArrowUpDown className="ml-2 h-4 w-4" />
                    </div>
                  </TableHead>
                  <TableHead className="cursor-pointer hidden lg:table-cell" onClick={() => toggleSort("job_title")}>
                    <div className="flex items-center">
                      Job Title
                      <ArrowUpDown className="ml-2 h-4 w-4" />
                    </div>
                    ...
                    <TableRow>
                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          <div className={`font-bold ${getScoreColor(result.similarityScores.overall)}`}>
                      ...
                      <TableCell className="hidden md:table-cell">
                        <div className={`font-medium ${getScoreColor(result.similarityScores.skills)}`}>
                          {formatPercentage(result.similarityScores.skills)}
                        </div>
                      </TableCell>
                      <TableCell className="hidden md:table-cell">
                        <div className={`font-medium ${getScoreColor(result.similarityScores.education)}`}>
                          {formatPercentage(result.similarityScores.education)}
                        </div>
                      </TableCell>
                      <TableCell className="hidden lg:table-cell">
                        <div className={`font-medium ${getScoreColor(result.similarityScores.job_title)}`}>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                              ...
                              <div className="flex flex-col gap-2">
                                {result.entityData?.Degree?.length > 0 ? (
                                  result.entityData.Degree.map((degree, index) => (
                                    <div key={index} className="text-sm">
                                      <Badge className="mr-2 bg-blue-100 text-blue-800 hover:bg-blue-100">Degree</Badge>
                                      {degree}
                                    </div>
                                  ))
                                ) : (
                                ...
                                {result.entityData?.["College Name"]?.length > 0 && (
                                  result.entityData["College Name"].map((college, index) => (
                                    <div key={index} className="text-sm">
                                      <Badge className="mr-2 bg-purple-100 text-purple-800 hover:bg-purple-100">Institution</Badge>
                                      {college}
                                    </div>
                                  ))
                                )}
                            ...
                            <div>
                              <h4 className="font-semibold mb-2">Identified Job Titles</h4>
                              <div className="flex flex-col gap-2">
                                {result.entityData?.["Job title"]?.length > 0 ? (
                                  result.entityData["Job title"].map((title, index) => (
                                    <div key={index} className="text-sm">
                                      <Badge className="mr-2 bg-green-100 text-green-800 hover:bg-green-100">Title</Badge>
                                      {title}
                                    </div>
                                  ))
                                ) : (
                                  <span className="text-sm text-muted-foreground">No job titles identified</span>
                                )}
                              </div>
                              
                              <h4 className="font-semibold mt-4 mb-2">Detailed Match Scores</h4>
                              <div className="space-y-3">
                                {Object.entries(result.similarityScores).map(([key, value]) => (
                                  <div key={key}>
